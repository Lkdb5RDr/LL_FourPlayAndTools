...
inline std::string cast(BSFixedString &v) { return v.c_str(); }
...
namespace Data {
	...
	*/
	...
	public:
		...
		// Serialization
		void Revert();
		void Save(const F4SESerializationInterface *intfc, UInt32 type);
		void Load(const F4SESerializationInterface *intfc, UInt32 &version, UInt32 &length);
	...
	public:
		...
		//template <> BSFixedString ListGet<BSFixedString, std::string>(UInt64 obj, std::string key, UInt32 index);
